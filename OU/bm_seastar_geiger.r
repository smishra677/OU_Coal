...
)
{
  ...
  model<-"BM"
  lik=bm.lik(phyloe,phy,dat,SE,model,...)
  ...
  ##z0
  mm=c(mm[argn], z0=0, mm[names(mm)[!names(mm)%in%argn]])
...
bm.lik=function(phyloe,phy, dat, SE = NA, model="BM", ...){
  if(length(model)==1){
    if(model=="trend") model<-"rate_trend"
    else if(model=="drift") model<-"mean_trend"
  }
...
fit_bm <- geiger::fitContinuous(tree, states, model="BM")
fit_bm



fit_bm <- fitContinuous1(C_matrix,tree, states, model="BM")
fit_bm$opt$alpha
fit_bm$opt$sigsq
fit_bm$opt$lnL
