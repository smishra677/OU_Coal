...
for (i in 1:length(C_matrix))
{
  ...
  {
    C_matrix[i]=exp(-2*alpha*(C_matrix[(floor(i/(n+1))*(n+1))+1]-C_matrix[i]+covariance_dissortion))*(1-exp(-2*alpha*(C_matrix[i]+2*N)))/(2*alpha)
  }
...
for (i in 1:length(C_matrix))
{
  if(i%%(n+1)==1)
  {
    C_matrix[i]= (1-exp(-2*alpha*(C_matrix[i]+expected_disortion)))/(2*alpha)
  }
}
...
write.csv(ee_k_ou, "C:\\Users\\smish\\Documents\\OU\\dataouou12.csv", row.names = FALSE)
write.csv(ee_k, "C:\\Users\\smish\\Documents\\OU\\databmou12.csv", row.names = FALSE)
